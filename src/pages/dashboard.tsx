...
export default function EventsPage() {
  ...
  async function handleDelete(id: string) {
    try {
      ...
      if (error) throw error;
      await setEvents((prevEvents) => prevEvents.filter((event) => event.id !== id));
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Dashboard Event</h1>
      <button onClick={() => router.push('add')} className="bg-green-500 text-white px-4 py-2 mb-4">
        Tambah Event
      </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {events.map(event => (
            <div key={event.id} className="border p-4 rounded-lg shadow-lg">
              ...
              <button onClick={() => router.push(`edit?id=${event.id}`)} className="bg-blue-500 text-white px-2 py-1 mt-2 mr-2">
                Edit
              </button>
              <button onClick={() => handleDelete(event.id)} className="bg-red-500 text-white px-2 py-1 mt-2">
                Hapus
              </button>
            </div>
          ))}
